...
if __name__ == "__main__":
    ...
    split_idx = int(INITIAL_SPLIT * len(dataset))
    ...
    training_args = TrainingArguments(
        use_cpu=True,
        output_dir='./output',
        eval_strategy="no",  # optional: also disable evaluation every epoch
        save_strategy="no",  # prevent checkpoint saving
        per_device_train_batch_size=16,
        per_device_eval_batch_size=16,
        num_train_epochs=1,
        weight_decay=0.01,
        # load_best_model_at_end=True,  <-- remove or comment out
    )

    # Store full classification reports
    reports = []

    # Store added citation for each round
    added_citations = {}
    ...
    for round_idx in range(AL_ROUNDS):
        ...
        added_citations[round_idx + 1] = newly_labeled.to_dict()
        print(added_citations)

    print(reports)

    with open('AL_V5_Adding_1_per_Round.pkl', 'wb') as f:
